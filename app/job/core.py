...
def jobfromparm(scheduler,**jobargs):
    ...
    cron = jobargs['cron']
    print('[add job]: ',id)
    scheduler.add_job(func=func,id=id, kwargs={'cmd':args,'task_id':id},trigger='cron',**cron,replace_existing=True)
    print('[add job ok]')
    return id

def get_job_logs(jid):
    if jid == None:
        data_list = TaskLog.query.filter_by().all()
    else:
